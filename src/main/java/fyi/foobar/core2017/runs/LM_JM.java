...
/**
 * LM (Jelinek-Mercer smoothing).
 * 
 * @author Stuart Mackie (stuart@foobar.fyi).
 * @version May 2020.
 */
public class LM_JM implements Runnable {

    private static final Logger logger = LogManager.getLogger(LM_JM.class);
    ...
    private static final String[] lambda = new String[] {"0.01", "0.05", "0.10", "0.15", "0.20",
    ...
    public LM_JM(Configuration config, IndexReader ir) {
        this.config = config;
        this.ir = ir;
    ...
    public void run() {
        ...
        for (String param : lambda) {

            StringBuilder run = new StringBuilder();

            logger.info("[" + textproc + "]" + " " + "LM (Jelinek-Mercer)" + "-" + param);

            IndexSearcher is = new IndexSearcher(ir);
            is.setSimilarity(new LMJelinekMercerSimilarity(Float.valueOf(param)));
            ...
            try {
                FileUtils.writeStringToFile(
                        new File(evaldir + "/" + "LM-JM" + "-" + param + "-" + textproc),
